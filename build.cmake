...
set (PROJECT_TYPE           7)
set (BUILD_TYPE             8)
set (GEN_CMAKELISTS         9)
set (CUSTOM_ARGS            10)
set (CUSTOM_ARGS_PD         11)
set (COMPILER_ARGS_DEBUG    12)
set (COMPILER_ARGS_RELEASE  13)
set (NUM_PROPS              14)
set (NUM_PROPS_MINUS_1      13)
set (NUM_PROPS_MINUS_2      9)
set (LIST_NAMES "TARGET_NAME;KRAL_PATH;PACKAGE_DIRS;PROJECTS_ROOT;PLATFORM;PROJECT_DIR;BUILD_DIR;PROJECT_TYPE;BUILD_TYPE;GEN_CMAKELISTS;CUSTOM_ARGS;CUSTOM_ARGS_PD;COMPILER_ARGS_DEBUG;COMPILER_ARGS_RELEASE")
...
macro (set_internal_check_vars VAL)
    ...
    set (processing_compiler_args_debug ${VAL})
    set (processing_compiler_args_release ${VAL})
endmacro()
...
function(add_target TNAME)
    ...
    while (value LESS ${ARGC})
        ...
        elseif ("${ARGV${value}}" STREQUAL "CUSTOM_ARGS_PD") 
            set_internal_check_vars(False)
        elseif ("${ARGV${value}}" STREQUAL "COMPILER_ARGS_DEBUG")
            set_internal_check_vars(False)
        elseif ("${ARGV${value}}" STREQUAL "COMPILER_ARGS_RELEASE")
            set_internal_check_vars(False)
        endif()
        ...
        elseif (processing_compiler_args_debug)
            set (acc_compiler_args_debug "${acc_compiler_args_debug}${ARGV${value}}")
        elseif (processing_compiler_args_release)
            set (acc_compiler_args_release "${acc_compiler_args_release}${ARGV${value}}")
        endif()
        ...
        elseif ("${ARGV${value}}" STREQUAL "COMPILER_ARGS_DEBUG") 
            set (processing_compiler_args_debug True)
        elseif ("${ARGV${value}}" STREQUAL "COMPILER_ARGS_RELEASE") 
            set (processing_compiler_args_release True)
        endif()
        math (EXPR value "${value} + 1")
    endwhile()
    ...
    if (NOT "${acc_compiler_args_debug}" STREQUAL "")
        set_target_value(${TNAME} ${COMPILER_ARGS_DEBUG} "${acc_compiler_args_debug}")
    endif()
    if (NOT "${acc_compiler_args_release}" STREQUAL "")
        set_target_value(${TNAME} ${COMPILER_ARGS_RELEASE} "${acc_compiler_args_release}")
    endif()
...
if ("${PLATFORM_${T}}" STREQUAL "android")
    ...
    string(REGEX MATCH "-DANDROID_NATIVE_API_LEVEL=(.*) " ANAL_MATCH ${CUSTOM_ARGS_PD_${T}})
    set (ANDROID_NATIVE_API_LEVEL ${CMAKE_MATCH_1})
    string(REGEX MATCH "id: ([0-9]+) or \"${ANDROID_NATIVE_API_LEVEL}\"" ALT_MATCH ${ALT_OUTPUT})
    set (ANDROID_TARGET "${CMAKE_MATCH_1}")
    set (CMAKE_GENERATED_COMMAND "${CMAKE_GENERATED_COMMAND} -DANDROID_TARGET=${ANDROID_TARGET} -DANDROID_APP_NAME=SDLDemo")
endif()
...
if (GENERATE OR G)
    ...
    set (CCFILE "${PROJECTS_ROOT_DIR}/${CURRENT_BUILD_DIR}/compiler_config.cmake")
    get_target_value(${T} ${COMPILER_ARGS_DEBUG} CC_ARGS_DEBUG)
    get_target_value(${T} ${COMPILER_ARGS_RELEASE} CC_ARGS_RELEASE)
    file (WRITE ${CCFILE} "")
    file (APPEND ${CCFILE} "set (CMAKE_CXX_FLAGS_DEBUG \"\${CMAKE_CXX_FLAGS_DEBUG} ${CC_ARGS_DEBUG}\")\n")
    file (APPEND ${CCFILE} "set (CMAKE_CXX_FLAGS_RELEASE \"\${CMAKE_CXX_FLAGS_RELEASE} ${CC_ARGS_RELEASE}\")\n")
