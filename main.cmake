...
macro(kral_init)
    ...
    foreach (package ${PACKAGES})
        string (FIND "${CMAKE_SOURCE_DIR}" "${package}" IS_PACKAGE_IN_PATH)
        if (NOT IS_PACKAGE_IN_PATH EQUAL -1)
            string (REPLACE "${package}/" "" LIB_TO_APPEND ${CMAKE_SOURCE_DIR})
            string (FIND "${LIB_TO_APPEND}" "/" DOUBLE_PATH)
            if (DOUBLE_PATH EQUAL -1)
                set (LIB_TO_APPEND "${LIB_TO_APPEND}/")
            endif()
            set (CURRENT_DEPENDENCY "${LIB_TO_APPEND}-lib-name" CACHE INTERNAL "${CURRENT_DEPENDENCY}" FORCE)
            list (APPEND CONSTRUCTED_LIBS "${LIB_TO_APPEND}-lib-name")
...
set (SOURCES_EXTENSIONS "*.c;*.cc;*.cpp;*.cxx;*.m;*.mm;*.ui;*.h;*.cu")
...
function (module PNAME)
    ...
    while (value LESS ${ARGC})
        ...
        elseif ("${ARGV${value}}" STREQUAL "ENABLE_CUDA")
    ...
    if (CUDA_ENABLED)
        ...
        if (CUDA_FOUND)
            cuda_add_executable (${PNAME} ${BUILD_OPTION} ${${PNAME}_F_INCLUDE} ${${PNAME}_F_SOURCES} ${${PNAME}_plaf_F_SOURCES})
        else()
            message (ERROR "CUDA package was not found!") 
        endif()
    else()
...
function (library PNAME)
    message(STATUS "Build library: ${PNAME}")
    set (value 1)
    while (value LESS ${ARGC})
        if ("${ARGV${value}}" STREQUAL "DEPENDENCY")
            math (EXPR value "${value} + 1")
